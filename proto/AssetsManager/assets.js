...
function run() {
    if (BABYLON.Engine.isSupported()) {
        ...
        var assetName = window.location.hash;
        assetName = assetName.replace('#','');
        if(!assetName){
            window.alert('why did you pass no asset name, use the hash # symbol to pass a filename');
        }

        var meshTask = assetsManager.addMeshTask("mesh task", "", "./", assetName);
